...
std::string PlayerbotDbStore::GetSingleValue(uint64 guid, std::string preset, std::string key)
{
    auto results = CharacterDatabase.PQuery(
        "SELECT `value` FROM `ai_playerbot_db_store` WHERE `guid` = '%lu' AND `preset` = '%s' AND `key` = '%s' ORDER BY `id` DESC LIMIT 1",
        guid, preset.c_str(), key.c_str());

    if (!results)
        return "";

    Field* fields = results->Fetch();
    return fields ? fields[0].GetString() : "";
}
